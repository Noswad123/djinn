...
is used. After exporting, you can remove source sessions with `session cleanup`
and the promotion session with `session rm` if you no longer need the provenance.
