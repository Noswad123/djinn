      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>djinn buddy {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  ...
  const url = new URL("https://github.com/Noswad123/djinn/issues/new")
  url.searchParams.set("title", `TUI crash: ${message}`)
  const metadata = `**Djinn Buddy version:** ${InstallationVersion}\n**OS:** ${describeOS()}\n**Terminal:** ${describeTerminal()}\n**Reproduce:** Reported automatically from the Djinn crash screen. If you can, describe what you were doing when it crashed.`
  ...
  const marker = "\n... (truncated)"
  const head = `The Djinn Buddy TUI crashed with an unexpected error.\n\n${metadata}\n\n**Error:** ${message}\n\n**Stack trace:**\n`
