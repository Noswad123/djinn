...
function withDjinnEvents<A, E, R>(eventsPath: string, fx: () => Effect.Effect<A, E, R>) {
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.DJINN_EVENTS_JSONL
      process.env.DJINN_EVENTS_JSONL = eventsPath
      return prev
    }),
    () => fx(),
    (prev) =>
      Effect.sync(() => {
        if (prev === undefined) delete process.env.DJINN_EVENTS_JSONL
        else process.env.DJINN_EVENTS_JSONL = prev
      }),
  )
}
...
it.instance("prompt appends Djinn events when DJINN_EVENTS_JSONL is set", () =>
  Effect.gen(function* () {
    const { dir, llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const chat = yield* sessions.create({ title: "Djinn events" })
    const eventsPath = path.join(dir, "djinn", "events.jsonl")
    yield* llm.text("hello from assistant")

    yield* withDjinnEvents(eventsPath, () =>
      prompt.prompt({
        sessionID: chat.id,
        agent: "build",
        parts: [{ type: "text", text: "hello djinn" }],
      }),
    )

    const lines = (yield* Effect.promise(() => Bun.file(eventsPath).text()))
      .trim()
      .split("\n")
      .map((line) => JSON.parse(line))
    expect(lines.map(({ type, content }) => ({ type, content }))).toEqual([
      { type: "user_message", content: "hello djinn" },
      { type: "assistant_message", content: "hello from assistant" },
    ])
    expect(lines[0].event_id.startsWith("buddy:user_message:msg")).toBe(true)
    expect(lines[1].event_id.startsWith("buddy:assistant_message:msg")).toBe(true)
  }),
)

it.instance("prompt skips Djinn events with an existing event_id", () =>
  Effect.gen(function* () {
    const { dir } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const chat = yield* sessions.create({ title: "Djinn event dedupe" })
    const eventsPath = path.join(dir, "djinn", "events.jsonl")
    const messageID = MessageID.ascending()
    const eventID = `buddy:user_message:${messageID}`
    yield* writeText(
      eventsPath,
      JSON.stringify({ event_id: eventID, type: "user_message", content: "hello djinn" }) + "\n",
    )
    ...
    const lines = (yield* Effect.promise(() => Bun.file(eventsPath).text())).trim().split("\n")
    expect(lines).toHaveLength(1)
    expect(JSON.parse(lines[0]).event_id).toBe(eventID)
  }),
)
