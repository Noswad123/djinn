...
type DjinnEventType = "user_message" | "assistant_message"

function textContent(parts: SessionV1.Part[], options?: { includeSynthetic?: boolean }) {
  return parts
    .filter((part): part is SessionV1.TextPart => part.type === "text")
    .filter((part) => !part.ignored)
    .filter((part) => options?.includeSynthetic || !part.synthetic)
    .map((part) => part.text)
    .join("\n\n")
}

function userEventContent(message: SessionV1.WithParts) {
...
function assistantEventContent(message: SessionV1.WithParts) {
  const text = textContent(message.parts, { includeSynthetic: true })
  if (text.trim()) return text
  if (message.info.role === "assistant" && message.info.structured !== undefined) return JSON.stringify(message.info.structured)
  return ""
}

function djinnEventID(type: DjinnEventType, messageID: string) {
  return `buddy:${type}:${messageID}`
}

function appendDjinnEvent(type: DjinnEventType, content: string, eventID: string) {
  const eventsPath = process.env.DJINN_EVENTS_JSONL
  if (!eventsPath || !content.trim()) return Effect.void

  return Effect.tryPromise({
    try: async () => {
      await mkdir(path.dirname(eventsPath), { recursive: true })
      const existing = await readFile(eventsPath, "utf8").catch((error) => {
      ...
      for (const line of existing.split("\n")) {
        ...
        if (!trimmed) continue
        try {
          const event = JSON.parse(trimmed) as { event_id?: unknown }
          if (event.event_id === eventID) return
        } catch {
          continue
        }
      }
      await appendFile(eventsPath, JSON.stringify({ event_id: eventID, type, content }) + "\n", "utf8")
    },
    catch: (error) => error,
  }).pipe(
    Effect.catch((error) =>
      Effect.logWarning("failed to append djinn event", {
        type,
        eventsPath,
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      yield* appendDjinnEvent("user_message", userEventContent(message), djinnEventID("user_message", message.info.id))
      ...
      const reply = yield* loop({ sessionID: input.sessionID })
      if (reply.info.role === "assistant" && reply.info.parentID === message.info.id) {
        yield* appendDjinnEvent(
          "assistant_message",
          assistantEventContent(reply),
          djinnEventID("assistant_message", reply.info.id),
        )
      }
      return reply
    })
